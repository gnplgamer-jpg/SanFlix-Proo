          ...
          >
            ...
            <div className={`p-4 sm:p-6 flex items-center justify-between bg-gradient-to-b from-black/80 to-transparent pointer-events-auto transition-opacity ${isLocked ? 'opacity-0' : 'opacity-100'} w-full overflow-x-auto gap-4 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]`}>
              <div className="flex items-center gap-2 sm:gap-3 shrink-0">
                 ...
                 <button 
                   onClick={(e) => { e.stopPropagation(); if (!isLocked) setVisualEnhancer(!visualEnhancer); }}
                   className={`bg-white/10 hover:bg-white/20 text-white p-2 rounded-full backdrop-blur-sm transition shadow-lg border border-white/10 flex items-center gap-2 px-3 ${visualEnhancer ? 'bg-indigo-500/30 border-indigo-500/50' : ''}`}
                 >
                   <Wand2 className={`w-4 h-4 ${visualEnhancer ? 'text-indigo-400' : 'text-gray-400'}`} />
