...
// 2. Add css filter to video element
const currentFilterCss = "filters.find(f => f.name === activeFilter)?.css || 'none'";
// Search for `<video` and add style
code = code.replace(
  /<video\s+ref=\{videoRef\}/,
  `<video\n            style={{ filter: \${currentFilterCss} }}\n            ref={videoRef}`
);

// 3. Add button in controls (Next to Settings / Quality)
const wandBtn = `
                    <button 
...
// 4. Add the Visual Enhancer Overlay/Panel
const enhancerPanel = `
      {/* Visual Enhancer Modal */}
      <AnimatePresence>
        {showVisualEnhancer && (
          <div className="fixed inset-0 z-[60] bg-black/90 flex flex-col pointer-events-auto" onClick={(e) => e.stopPropagation()}>
            {/* Header */}
            <div className="flex items-center px-4 py-4 border-b border-zinc-800 bg-zinc-950/80 backdrop-blur-md">
              <button onClick={() => setShowVisualEnhancer(false)} className="text-white p-2">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                {filters.map(filter => (
                  <div 
                    ...
                    className={\`relative rounded-2xl overflow-hidden cursor-pointer group border-2 transition-all \${activeFilter === filter.name ? 'border-amber-500 shadow-lg shadow-amber-500/20' : 'border-transparent hover:border-zinc-500'}\`}
                  >
                    <div className="aspect-[16/9] relative">
                      ...
                      <div className="absolute bottom-2 left-2 right-2 text-center">
                        <span className={\`font-bold text-sm \${activeFilter === filter.name ? 'text-amber-400' : 'text-white'}\`}>{filter.name}</span>
                      </div>
            ...
            <div className="p-4 border-t border-zinc-800 bg-zinc-950/80 backdrop-blur-md">
               <button 
                 onClick={() => setShowVisualEnhancer(false)}
                 className="w-full py-4 rounded-xl font-bold text-white bg-zinc-800 hover:bg-zinc-700 transition"
               >
                 Apply to all & Close
               </button>
            </div>
...
console.log('patched direct video player');
